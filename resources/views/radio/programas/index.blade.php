    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Listado de Programas de la Radio') }}
            </h2>
            <a href="{{ route('radio.programas.create') }}"
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    @foreach ($programas as $programa)
                        <div x-data="{ showModal: false }"
                            ...
                            <!-- Imagen opcional -->
                            <div class="block w-full aspect-[16/9] bg-gray-100 overflow-hidden">
                                @if ($programa->imagen)
                                ...
                                @else
                                    <img src="https://via.placeholder.com/640x360.png?text=Sin+Imagen"
                                        alt="Placeholder"
                            ...
                            <div class="p-5 flex-1 flex flex-col justify-between">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600 mb-2">
                                        Conductor: <span class="font-semibold">{{ $programa->conductor ?? '—' }}</span>
                                    </p>
                                ...
                                <div class="flex items-center justify-between border-t border-gray-100 pt-4">
                                    <div class="text-xs text-gray-500">
                                        <!-- Si quieres, aquí puedes poner fecha de creación -->
                                        Creado: {{ $programa->created_at->format('d/m/Y') }}
                                    </div>
                                    <div class="flex gap-2 text-sm">
                                        <!-- Botón Ver -->
                                        <a href="{{ route('radio.programas.show', $programa) }}" title="Ver"
                                        ...
                                        <!-- Botón Editar -->
                                        <a href="{{ route('radio.programas.edit', $programa) }}" title="Editar"
                                            class="p-2 text-gray-500 hover:text-blue-600 transition duration-200">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-7 1.5l2-2m-2 2l-2 2m4-2l2-2M15 7l3 3m0 0l-3 3m3-3h-4.5M16 5l-2 2m2-2l2 2m-2-2L14 3m0 0L12 5"/>
                                            </svg>
                                        </a>
                                        <!-- Botón Eliminar -->
                            ...
                            <div x-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                                <div class="bg-white rounded-lg shadow-lg w-96 p-6" @click.away="showModal = false">
                                    ...
                                    <p class="mb-6 text-gray-600">¿Estás seguro de que quieres eliminar este programa? Esta acción no se puede deshacer.</p>
                                    <div class="flex justify-end gap-3">
                                        <button @click="showModal = false"
                                            class="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300">
                                            Cancelar
                                        </button>
                                        <form action="{{ route('radio.programas.destroy', $programa) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
            ...
            @else
                <div class="text-center py-16 px-4 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                    <h3 class="mt-4 text-xl font-semibold text-gray-900">No hay programas</h3>
                    <p class="mt-2 text-sm text-gray-500">
                        Comienza creando tu primer programa para que aparezca aquí.
                    </p>
                    <div class="mt-6">
                        <a href="{{ route('radio.programas.create') }}"
